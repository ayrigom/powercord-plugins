...
module.exports = class CustomActivity extends Plugin {
  ...
  async findAndModifyActivity () {
    const getActivityUrl = await getAllModules((m) =>
      m?.default?.toString()?.match(/if\(.\.default\.inTestModeForEmbeddedApplication\(.\)\)return .\.default\.testModeOriginURL;/)
    )[0];
    ...
    inject('custom-activity', getActivityUrl, 'default', (args, res) => {
      ...
      if (this.settings.get(`activity-${activityId}`, `https://${activityId}.discordsays.com`) === `https://${activityId}.discordsays.com`) {
        return res;
      }
